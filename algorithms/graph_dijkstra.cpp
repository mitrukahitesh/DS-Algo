...
void Graph::dijkstra(int s) {
	...
	while (!heap->empty()) {
		...
		for (auto it = array[u].begin(); it != array[u].end(); ++it) {
			int v = (*it).getVertex();
			int w = (*it).getWeight();
			if (heap->isinHeap(v) && distances[u] != INT_MAX && w + distances[u] < distances[v]) {
...
int main() {
	...
	cout << "Following is Single source all destination for weighted graph with non-negative edges"
