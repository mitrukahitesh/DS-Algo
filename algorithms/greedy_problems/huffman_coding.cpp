...
class comparator {
   public:
    bool operator()(
        Node* l, Node* r) {  // return true if we want l to be placed before r
        return l->freq > r->freq;  // the element popped is the last according
                                   // to this strict weak ordering
    }  // in priority_queue
...
void printcodes(Node* head, string s) {
    ...
    if (head->value != '\0') cout << head->value << " : " << s << endl;
