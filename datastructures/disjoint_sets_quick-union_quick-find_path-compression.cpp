...
//- means node is a root and the value with - represents total nodes includeing
//root
...
int find(char* elements, int* parent, int n, char c) {
    ...
    // path compression
    // point all members along just found path to the root for faster access in
    // future
    while (temp1 != i) {
        int temp2 = temp1;
        temp1 = parent[temp1];
...
void Union(char* elements, int* parent, int n, char c1, char c2) {
    int i, j;
    if (((i = find(elements, parent, n, c1)) !=
         (j = find(elements, parent, n, c2)))) {
