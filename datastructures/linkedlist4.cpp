...
void reverse_node(){
	...
	while(temp1!=NULL){
		...
		prev = temp1;           //sets current node to previous for which is stores in (next) of next node
...
void print_recursive(node* p){
	...
	{
		cout<<p->data<<" ";           //first print then recursive call to print in normal order
...
void print_reverse_recursive(node* p){
	...
	{
		print_reverse_recursive(p->next);  //first call then print to print in reverse order
		cout<<p->data<<" ";
	}
}

void reverse_by_recursion(node* p){
	if(p->next==NULL){
		head = p;
		return;
	}
...
int main(){
	...
	reverse_by_recursion(for_recursion); 
	print(); //0 1 2 4 5 
